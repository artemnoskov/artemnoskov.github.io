$( document ).ready(function() {
  $( "div.dialog" ).append('<div class="card" style="width: 90%"><div class="card-body text-justify"><div class="card-header text-center"><h5>Внимание!</h5></div><p class="card-text">Cайт позволяет сгенерировать <b>код для отправки заявки</b> на получение цифрового пропуска по СМС. То, что вы получите по окончанию заполнения формы <b>не является пропуском!</b></p><p class="card-text">Для оформления заявки вы должны будете скопировать полученный код (сообщение) и <b>самостоятельно отправить его на номер <i>7377</i></b>. После этого вы получите ответ по СМС.</p><p class="card-text">Мы не собираем и не храним введенную вами информацию.</p><a href="#" id="start" class="btn btn-primary btn-block">Продолжить</a></div></div>');
});
...
function createSMS(){
  ...
  if((navigator.userAgent.match(/Android/i)) || (navigator.userAgent.match(/iPhone/i))) {
    $('.resMSG').append('<button class="btn btn-success" id="sendSMS" type="submit">Отправить</button>');
  }

  $('.resMSG').append('<button class="btn btn-primary" id="copySMS" type="submit">Скопировать</button>');
